...
class Test {
    ...
    async getIp() {
        ...
        await fs.readFile('../ip_address', function (err, data) {
            if (err) {
                ...
                console.log("Please creat a file called ip_address that contains the private ip address")
    ...
    async httpRequest(url, method, payload) {
    ...
    async createSchema() {
        let schema_name = name + "-Ticket" + Math.round(Math.random() * 1000);
        let schemaBody = {
            "attributes": [
                "Departure:",
                "Destination:",
        ...
        let response = await this.httpRequest(agentUrl + "/schemas", "POST", schemaBody).catch(err => {
            console.log(err);
            return Promise.reject(-1);
        });
        console.log(response);
    ...
    async createCredDef() {
        ...
        let response = await this.httpRequest(agentUrl + "/credential-definitions", "POST", credDefBody).catch(err => {
            console.log(err);
            return Promise.reject(-1);
        });
        console.log(response);
    ...
    async getInvitation() {
        ...
        let response = await this.httpRequest(agentUrl + "/connections/create-invitation?auto_accept=true", "POST", {}).catch(err => {
            console.log(err)
            return Promise.reject(-1)
        });
    ...
    async issueCredential(connectionId, start, dest, time) {
        ...
        let response = await this.httpRequest(agentUrl + "/issue-credential/send", "POST", credentialBody).catch(err => {
            console.log(err);
            return Promise.reject(-1);
        });
        console.log(response);
                ...
                if (req.method == "GET") {
                    ...
                    if (req.url == "/topic/connections/") {
                        ...
                        req.on('error', (err) => {
                            ...
                            if (json_body["state"] == "response") {
                                console.log("Continuing with issue credential");
                                this.issueCredential(body.connection_id);
                            } else {
                                console.log("Nothing to do");
                            }
                            res.writeHead(200, "Ok");
